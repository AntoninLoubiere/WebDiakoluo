...
const editColumnModalScore = document.getElementById('modal-edit-column-settings-score');
const editColumnModalScoreAdd = document.getElementById('modal-edit-column-settings-score-add');
var editColumnModalScoreMax;
...
class EditPage extends Page {
    ...
    I18N.initAsyncFunc.then(() => {
        editColumnModalScoreMax = document.getElementById('modal-edit-column-score-max');
        editColumnModalScoreMax.onchange = this.onScoreMaxChange.bind(this);
    });
    }
        ...
        if (currentTest.editSync) {
            ...
            if (currentTest.sync) {
                SyncManager.getSyncFromTest(currentTest).then(s => {
                    ...
                    editSyncSyncId.name = s.sync.serverTestId;
                });
            } else {
                delete this.sync;
            }
    ...
    updateColumnModal(id, addHistory=true) {
        ...
        if (editColumnModal.id != id) {
            if (this.scoreRules) {
                this.scoreRules.forEach(s => s.removeView());
            }
            this.scoreRules = [];
            ...
            const column = currentTest.columns[id];
            ...
            this.scoreSelectTemplate = document.createElement('select');
            for (let r of column.getAllowedScoreFunctions()) {
                const scoreRule = SCORE_REGISTRATION[r];
                let op = document.createElement('option');
                op.value = op.id = r;
                op.textContent = I18N.getTranslation(scoreRule.m_t);
                op.title = I18N.getTranslation(scoreRule.m_m);
                this.scoreSelectTemplate.appendChild(op);
            }
            let op = document.createElement('option');
            op.value = op.id = 'special';
            op.textContent = I18N.getTranslation('score-remove-rule');
            this.scoreSelectTemplate.appendChild(op);

            editColumnModalScoreMax.value = column.scoreRules.max;

            this.addScoreRuleSelect = this.scoreSelectTemplate.cloneNode(true);
            let addOption = this.addScoreRuleSelect.options.namedItem('special');
            addOption.textContent = I18N.getTranslation('score-add-rule');
            addOption.disabled = true;
            this.addScoreRuleSelect.value = 'special';
            this.addScoreRuleSelect.onchange = () => {
                if (this.addScoreRuleSelect.value == 'special') return;

                let rules = column.scoreRules.rules;
                let r = !SCORE_REGISTRATION[this.addScoreRuleSelect.value].multiple && rules.find(r => r.type == this.addScoreRuleSelect.value);
                if (!r) {
                    r = {type: this.addScoreRuleSelect.value, score: 1};
                    rules.push(r);
                }

                let scoreRule = new ScoreRule(this, r);
                this.scoreRules.push(scoreRule);
                scoreRule.create();

                this.resortScoreRulesArray();
                this.addScoreRuleSelect.value = 'special';
            }
            editColumnModalScoreAdd.replaceChild(this.addScoreRuleSelect, editColumnModalScoreAdd.firstChild)

            for (let r of column.scoreRules.rules) {
                let scoreRule = new ScoreRule(this, r);
                this.scoreRules.push(scoreRule);
                scoreRule.create();
            }
    ...
    resortScoreRulesArray() {
        let rules = currentTest.columns[editColumnModal.id].scoreRules.rules;
        rules.sort(this.resortFunc);
        for (let index = rules.length - 1; 0 <= index; index--) {
            const r = rules[index];
            editColumnModalScore.insertBefore(
                this.scoreRules.find(sr => sr.rule == r).view,
                editColumnModalScore.firstElementChild
            )
        }
    }

    maybeResortScoreRules(rule) {
        let rules = currentTest.columns[editColumnModal.id].scoreRules.rules;
        let i = rules.findIndex(r => rule == r);
        if ((i > 0 && this.resortFunc(rules[i - 1], rule) > 0) ||
            (i < rules.length - 2 && this.resortFunc(rule, rules[i + 1]) > 0))
        {
            this.resortScoreRulesArray();
        }
    }
    ...
    onScoreMaxChange() {
        let value = Number(editColumnModalScoreMax.value);
        currentTest.columns[editColumnModal.id].scoreRules.max = value;
        this.scoreRules.forEach(r => r.updateMax(value));
    }
...
class ScoreRule {
    constructor(editPage, rule) {
        this.editPage = editPage;
        this.rule = rule;
        this.scoreRule = SCORE_REGISTRATION[this.rule.type];
    }

    create() {
        this.view = document.createElement('li');
        this.container = document.createElement('div');
        this.container.classList.add('edit-settings-score-item');

        if (this.rule.type == 'fail') {
            let sp = document.createElement('span');
            sp.textContent = I18N.getTranslation(this.scoreRule.m_t);
            this.container.appendChild(sp);
        } else {
            this.select = this.editPage.scoreSelectTemplate.cloneNode(true);
        ...
        this.score.size = 2;
        this.score.onchange = () => {
            this.rule.score = Number(this.score.value);
            this.editPage.maybeResortScoreRules(this.rule);
            this.score.focus();
        }
        this.container.appendChild(this.score);

        this.maxPoints = document.createElement('span');
        this.maxPoints.textContent = ` / ${editColumnModalScoreMax.value}`;
        this.container.appendChild(this.maxPoints);

        let points = document.createElement('x-i18n');
        points.setAttribute('key', 'points');
    ...
    update() {
        this.score.value = this.rule.score;

        if (!this.select) {
            return;
        }
        this.select.value = this.rule.type;
        if (!this.scoreRule.multiple)
            this.editPage.scoreRules.forEach(i => i.registerNewScoreRule(this.rule.type));

        if (this.parametersView) {
            this.parametersView.remove();
        }

        if (this.scoreRule.parameters && this.scoreRule.parameters.length) {
            this.parametersView = document.createElement('div');

            for (let param of this.scoreRule.parameters) {
                switch (param.type) {
                    case 'select': {
                        let view = document.createElement('select');

                        for (let op of param.options) {
                            let opView = document.createElement('option');
                            opView.value = op.value;
                            opView.textContent = I18N.getTranslation(op.text);
                        ...
                        if ('p_' + param.name in this.rule) {
                            view.value = this.rule['p_' + param.name];
                        } else {
                            this.rule['p_' + param.name] = view.value;
                        }
                        view.onchange = () => this.rule['p_' + param.name] = view.value;
                        this.parametersView.appendChild(view);
                        break;
                    }
                }
            }

            this.container.insertBefore(this.parametersView, this.score);
        }
    }

    onSelectChange() {
        if (this.select.value == 'special') {
            this.remove();
            return;
        }
        this.setFromRuleType(this.select.value);
        this.editPage.maybeResortScoreRules(this.rule);
    }

    setFromRuleType(ruleType) {
        if (ruleType != this.rule.type) {
            if (!this.scoreRule.multiple)
                this.editPage.scoreRules.forEach(i => i.registerRemoveScoreRule(this.rule.type));
            if (this.scoreRule.parameters) {
                for (let param of this.scoreRule.parameters) {
                    delete this.rule['p_' + param.name];
                }
            }
            this.rule.type = ruleType;
            this.scoreRule = SCORE_REGISTRATION[ruleType];
            this.update();
        }
    }

    registerNewScoreRule(r) {
        if (this.select) {
            if (r == this.rule.type) {
                this.editPage.scoreSelectTemplate.options.namedItem(r).disabled = true;
                this.editPage.addScoreRuleSelect.options.namedItem(r).disabled = true;
            } else {
                this.select.options.namedItem(r).disabled = true;
            }
        }
    }

    registerRemoveScoreRule(r) {
        if (this.select) {
            if (r == this.rule.type) {
                this.editPage.scoreSelectTemplate.options.namedItem(r).disabled = false;
                this.editPage.addScoreRuleSelect.options.namedItem(r).disabled = false;
            } else {
                this.select.options.namedItem(r).disabled = false;
            }
        }
    }

    updateMax(value) {
        this.maxPoints.textContent = ` / ${value}`;
    }

    remove() {
        this.editPage.scoreRules.forEach(i => i.registerRemoveScoreRule(this.rule.type));
        let rules = currentTest.columns[editColumnModal.id].scoreRules.rules;
        let index = rules.findIndex(r => r.type == this.rule.type);
        if (index >= 0) {
            rules.splice(index, 1);
        }
        this.editPage.scoreRules.splice(this.editPage.scoreRules.findIndex(r => r == this), 1);
        this.removeView();
    }
