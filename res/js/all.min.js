...
function modalShowCheck(id){localStorage.setItem(id,document.getElementById('modal-show').checked);}var LANGUAGES=['en','fr'];var translations=null;var universal=null;var languageSelector=document.getElementById("language-selector");class I18nElement extends HTMLElement{static get observedAttributes(){return['key'];}
...
function initialise(){customElements.define('x-i18n',I18nElement);let request=new XMLHttpRequest();request.open('GET','/WebDiakoluo/res/translations/universal.json');request.responseType='json';request.send();request.onload=function(){universal=request.response;onSetLang();}
...
function setLang(lang,updateButton){let request=new XMLHttpRequest();request.open('GET','/WebDiakoluo/res/translations/'+lang+'.json');request.responseType='json';request.send();request.onload=function(){if(request.response==null){if(translations==null){setLang(detectLang());}}else{translations=request.response
onSetLang();}}
